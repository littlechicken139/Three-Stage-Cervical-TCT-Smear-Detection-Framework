...
class FlexIncConv(nn.Module):
    """Flexible Inception Convolution for multi-scale feature extraction."""
...
class FlexIncBlock(nn.Module):
    """Flexible Inception Block with multi-scale convolutions and GLU MLP."""
    def __init__(self, dim, drop_path=0.0):
        ...
        for ks in [3, 5]:
            self.convs.append(FlexIncConv(min_ch, ks, ks * 3 + 2))
        self.conv_1x1 = Conv(dim, dim, k=1)
    ...
    def forward(self, x):
        ...
        x_mixer = torch.cat([self.convs[i](x_group[i]) for i in range(len(self.convs))], dim=1)
        x_mixer = self.conv_1x1(x_mixer)
        
        x = x + self.drop_path(self.layer_scale_1.unsqueeze(-1).unsqueeze(-1) * x_mixer)
        x = x + self.drop_path(self.layer_scale_2.unsqueeze(-1).unsqueeze(-1) * self.mlp(self.norm2(x)))
        return x


class FlexInc_C3k2(C3k2):
    """Flexible Inception C3k2 block."""
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(FlexInc_C3k(self.c, self.c, 2, shortcut, g) if c3k else FlexIncBlock(self.c) for _ in range(n))


class FlexInc_C3k(C3k):
    """Flexible Inception C3k block."""
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=3):
        super().__init__(c1, c2, n, shortcut, g, e, k)
        c_ = int(c2 * e)
        self.m = nn.Sequential(*(FlexIncBlock(c_) for _ in range(n)))
# ================================ FlexInc Modules ================================
