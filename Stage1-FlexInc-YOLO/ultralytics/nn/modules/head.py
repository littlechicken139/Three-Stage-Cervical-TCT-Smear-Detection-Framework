...
class MSCA_Detect(Detect):
    """YOLOv8 Detect head with MSCA attention for detection models."""

    def __init__(self, nc=80, reg_max=16, end2end=False, ch=()):
        """Initializes the YOLOv8 detection layer with MSCA attention."""
        super().__init__(nc, reg_max, end2end, ch)
        # 重定义 cv2 和 cv3，使用 MSCA 模块
        c2, c3 = max((16, ch[0] // 4, self.reg_max * 4)), max(ch[0], min(self.nc, 100))
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 3), MSCA(c2, c2, 1), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch)
        self.cv3 = nn.ModuleList(nn.Sequential(nn.Sequential(DWConv(x, x, 3), Conv(x, c3, 1)), MSCA(c3, c3, 1), nn.Conv2d(c3, self.nc, 1)) for x in ch)
